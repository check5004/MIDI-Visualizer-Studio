...
4. **初回起動時の注意**:
   - 本アプリはAppleの開発者署名を行っていないため、通常のダブルクリックでは起動できない場合があります。
   - **「悪質なソフトウェアかどうかをAppleでは確認できないため、このソフトウェアは開けません」** というエラーが表示される場合：
     1. アプリアイコンを **右クリック（またはControlキーを押しながらクリック）** し、メニューから「開く」を選択してください。
     2. 確認ダイアログが表示されたら、再度「開く」をクリックすることで起動できます。
   - 上記の方法でも開けない場合、または「壊れている」と表示される場合は、ターミナルで以下のコマンドを実行してセキュリティ属性を削除してください：
     ```bash
     xattr -cr /path/to/midi_visualizer_studio.app
     ```
     （※ `/path/to/...` の部分は、実際のアプリファイルの場所（パス）に置き換えてください。ターミナルにアプリをドラッグ＆ドロップするとパスが入力されます）

## CI/CD
GitHub Actionsにより以下のワークフローが自動化されています：
- Web版のビルドとGitHub Pagesへのデプロイ。
- Windows版およびmacOS版のビルドとGitHub Releasesへの公開。
